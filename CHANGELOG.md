...
## v2.1.3 (2026-05-22)
### 🐛 修复
- 修复在旧版bash中，中文变量名可能导致程序出错的bug
- 修复安装脚本在sha256校验工具缺失时，错误显示校验通过的问题；现在会提示校验被跳过
### 📚 文档
- 更新变更日志以记录v2.1.3版本的更新
### 🔧 杂项
- 更新README中的示例版本号至2.1.4
...
- 安装时自动清理临时文件，简化设置过程。
- 网络连接测试失败时，提供重新配置、强制写入或退出三个选项，避免输入丢失。
...
### 🐛 修复
- 修复了程序退出时变量未定义导致的错误
- 修复了安装过程中的一键安装失败、版本选择错误和凭证替换问题
- 修正了版本选择、配置合并、输入处理和路径解析的问题
- 修复了代码审查中发现的多个错误，包括凭证泄露和写入问题
- 修复了自动化流程中命令解析错误的问题

### 📚 文档
- 更新了变更日志文件以匹配版本 v2.1.1

### 🔧 杂项
- 提升了自动化运行器的 CPU 性能至 2 核
- 更新了 README 文档中的构建示例版本号至 2.1.2

## v2.1.1 (2026-05-21)

### ♻️ 重构
- 修复了配置文件读取、显示对齐与运行稳定性方面的一些细节问题
- 现在如果配置文件读取出错，会明确提示问题，而不是悄悄覆盖
- 限制了更新检查接口的响应大小，与其他接口保持一致
- 修复了中英文混合时界面文字对不齐的问题
- 调整了模型列表的展示顺序，使其更加稳定

### 🐛 修复
- 修复了界面中模型输入框标题重复显示的问题
- 清理了界面上多余的提示文字
- 修复了检查更新时对特定版本号的识别问题
- 切换模型组合后，旧的认证信息会被正确清理，避免残留
- 优化了部分后台操作的流程，使其更稳定可靠
...
## v2.0.9 (2026-05-21)
### ✨ 新功能
- 新增一键安装脚本 `install.sh` / `install.ps1` / `install.cmd`，支持 `curl | bash` 一键安装
- 配置中模型名以 `-cc` 结尾时自动归类为 Anthropic 路由
### 🐛 修复
- 修复 `curl | bash` 管道执行时 stdin EOF 导致交互失败
- 修复跨平台兼容性的 5 项问题
### 📚 文档
- README 全面重写以与代码实际行为对齐
- 更新安装方式为一键 curl 安装
